...
run_deseq <- function(counts, samples, design = ~ trt, human = TRUE, filt_threshold = 1, top = 40, blind = TRUE, font_size = 4) {
  ...
  for (i in 1:length(vars)) {
    ...
    if (!is.data.frame(res)) {
      ...
      for (j in 1:choose(length(levels(dds[[vars[[i]]]])), 2)) {
        ...
        volcano <- plotly::ggplotly(ggplot2::ggplot(data = res1, aes(x = log2FoldChange, y = -log10(padj), group = gene_name)) + geom_point(aes(color = Result), size = 0.5) + xlab('log2 Fold Change') + ylab('-log10 p-value') + scale_color_manual(values = c('blue','red'), name = 'Result'))
      ...
      volcano <- plotly::ggplotly(ggplot2::ggplot(data = res, aes(x = log2FoldChange, y = -log10(padj), group = gene_name)) + geom_point(aes(color = Result), size = 0.5) + xlab('log2 Fold Change') + ylab('-log10 p-value') + scale_color_manual(values = c('blue','red'), name = 'Result'))
      ...
      ## Convert each filtered result table into normalized count tables for plotting
      x <- top_counts(res1, rld, filter = FALSE) %>% as_matrix()
      x <- x - rowMeans(x)
      n <- max(abs(range(x)))
      ## Create annotation table from colData, convert to data frame and create heatmap
      annot <- attr(dds, 'colData')[, vars, drop = FALSE] %>% as.data.frame()
